...
class PromptLearner(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        pos_tokenized_prompts = torch.cat([clip.tokenize(p) for p in pos_prompts]).to(device)
        neg_tokenized_prompts = torch.cat([clip.tokenize(p) for p in neg_prompts]).to(device)
        with torch.no_grad():
